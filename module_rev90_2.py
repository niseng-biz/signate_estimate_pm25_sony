...
class config:
    ...
    #明らかに遠い観測点を除去
    #DropCitylist = ['Novosibirsk','Darwin', 'Perth','Ürümqi', 'Naha', 'Calama', 'Sapporo', 'Saint Petersburg', 'Hegang', 'Bandar Abbas', 'Yazd']
...
def make_dailyslice():
    ...
    for timestamp_val in train_data['timestamp'].unique():
        ...
        # ファイルの存在確認
        #if os.path.isfile(data_path + 'train_dailyslice_save_'+pd.to_datetime(timestamp_val).strftime('%Y-%m-%d') +'_yt.csv') and os.path.isfile(data_path + 'test_dailyslice_save_'+pd.to_datetime(timestamp_val).strftime('%Y-%m-%d') +'_yt.csv') :
        if os.path.isfile(data_path + 'train_dailyslice_save_'+pd.to_datetime(timestamp_val).strftime('%Y-%m-%d') +'.csv') & os.path.isfile(data_path + 'test_dailyslice_save_'+pd.to_datetime(timestamp_val).strftime('%Y-%m-%d') +'.csv') :
            ...
            #_df_train_yt = train_data.loc[((train_data['timestamp'] == timestamp_val) | (train_data['timestamp'] == (timestamp_val-datetime.timedelta(days=1))) | (train_data['timestamp'] == (timestamp_val+datetime.timedelta(days=1)))), :]

            #_df.to_csv('train_data_slice_'+timestamp_val.strftime('%Y-%m-%d')+'.csv')
            for city_name in _df_train['City']:
                ...
                # 周辺領域のデータをきりだし
                #_df2 = make_sub_df(_df_train_yt, lon_this_city, lat_this_city, lat_bound_init, lon_bound_init)
            ...
            #train_data.loc[train_data['timestamp'] == timestamp_val, :].to_csv(data_path + 'train_dailyslice_save_'+pd.to_datetime(timestamp_val).strftime('%Y-%m-%d') +'_yt.csv')
            #test_data.loc[test_data['timestamp'] == timestamp_val, :].to_csv(data_path + 'test_dailyslice_save_'+pd.to_datetime(timestamp_val).strftime('%Y-%m-%d') +'_yt.csv')
...
def Preprocessing( train_data, test_data, cluster_data):
    ...
    #train_data = train_data.assign(weeknum=lambda d: (d['year'].astype(str) + '-' + d['month'].astype(str)))

    test_data = test_data.assign(timestamp=lambda d: pd.to_datetime(d['year'].astype(str) + '-' + d['month'].astype(str) + '-' + d['day'].astype(str)))
    test_data = test_data.assign(weeknum=[str(x.isocalendar()[0]) + '-' + str(x.isocalendar()[1])  for x in test_data['timestamp'].tolist()])
    #test_data = test_data.assign(weeknum=lambda d: (d['year'].astype(str) + '-' + d['month'].astype(str)))
    ...
    #corr_top10 = high_corr[0:11].tolist()
    print(corr_top10)
    ...
    SelectedFeature = corr_column(df_, config.drop_soukan)
    df_2 = train_data[SelectedFeature]
    dispname = [x.split(":")[0] for x in df_2.columns.values]
    ...
    SelectedFeature = SelectedFeature.insert(-1,'year')
    SelectedFeature = SelectedFeature.insert(-1,'month')
    SelectedFeature = SelectedFeature.insert(-1,'day')
    SelectedFeature = SelectedFeature.insert(-1,'lat')
    SelectedFeature = SelectedFeature.insert(-1,'clu_lat')
    SelectedFeature = SelectedFeature.insert(-1,'City')
    SelectedFeature = SelectedFeature.insert(-1,'clu_num')
    SelectedFeature = SelectedFeature.insert(-1,'Country')
    SelectedFeature = SelectedFeature.insert(-1,'timestamp')
    SelectedFeature = SelectedFeature.insert(-1,'weeknum')
    ...
    #tmp_train = np.repeat(np.nan, train_data.shape[0])
    #tmp_test = np.repeat(np.nan, test_data.shape[0])
    ...
    for c in train_data['clu_num_month'].unique():
        ...
        #tmp_test[test_index] = mean_val_tmp

        #n_split = min(4, len(train_data.loc[train_data['clu_num_month'] == c,[config.target]]))

        sub_df = train_data.loc[train_data['clu_num_month'] == c,:]

        #tmp_array = np.nan(len(train_data))

        #for idx_1, idx_2 in kf.split(train_data.loc[train_data['clu_num_month'] == c,:]):
        for idx_1, idx_2 in kf.split(sub_df):
            ...
            #print(mean_val_tmp)
            #for x in id_test:
            #    mean_val_tmp = mean_val_tmp +  train_data.loc[train_data['id'] == x,[config.target]].values
            mean_val_tmp = mean_val_tmp / len(id_test)

            for x in id_valid: train_data.loc[train_data['id'] == x,['clu_num_month']] = mean_val_tmp

            #tmp_array[train_data['id'] == x] = mean_val_tmp for x in id_valid
            #tmp_train[train_index] = mean_val_tmp
    ...
    #train_data['clu_num_month'] = tmp_train
    #test_data['clu_num_month'] = tmp_test
    ...
    #SelectedTrainData = train_data[SelectedFeature]
    #SelectedTestData = test_data[SelectedFeature.drop(config.target)]
    ...
    #categorical_columns = ['City', 'latlong']
    agg_country_value = dict([(c, [np.mean, np.max, np.min, np.std]) for c in corr_top10])

    #print('agg_country_value is ')
    #print(agg_country_value)
    ...
    #drop_columns_list = ['year', 'month', 'day', 'weekday'];
    #drop_columns_list = ['year', 'month', 'day'];
    if config.foptuna == 1:
...
def read_preprocessed_data():
    ...
    train_X = pd.read_csv(data_path + 'train_X_rev78.csv', index_col = 0)
    train_y = pd.read_csv(data_path + 'train_y_rev78.csv', index_col = 0)
    test_X  = pd.read_csv(data_path + 'test_X_rev78.csv', index_col = 0)
...
def fit_lgb(x, y, cv, model_params, fit_params, fobj=None, feval=None):
    ...
    for trn_idx, val_idx in cv:
        ...
        #x_train, x_valid = x.loc[trn_idx].values, x.loc[val_idx].values
        #y_train, y_valid = np.array(y.loc[trn_idx]), np.array(y.loc[val_idx])
...
def Traning(df_train, target):
    ...
    #gkf_cv = group_kfold_split(df_train, config.kfoldfeature, n_splits=config.nfold, random_state=config.random_seed)
    ...
    lgb_fit_params ={#'num_boost_rounds': 10000,
                     'num_boost_rounds': num_boost_rounds_set,
